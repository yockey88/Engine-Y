...
static constexpr uint32_t perlin_array[256] = {
...
namespace EngineY {
    ...
    float Noise2D::PerlinNoise(const glm::vec2& pos , GeneratorArgs args) {
        glm::vec2 p = glm::vec2(pos.x * args.frequency , pos.y * args.frequency);
        return glm::perlin(p) * args.amplitude;
    }

    float Noise2D::FractalBrownianMotion(const glm::vec2& pos , GeneratorArgs args) {
        float result = 0.f;
        float amp = args.amplitude;
        float freq = args.frequency;

        for (uint32_t i = 0; i < args.octaves; ++i) {
            float n = glm::perlin(pos * freq) * amp;
            result += n;

            amp *= args.persistence;
            freq *= 2.f;
        }
