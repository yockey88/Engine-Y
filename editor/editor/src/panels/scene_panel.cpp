...
namespace editor {
    ...
    void DrawTransformComponent(YE::components::Transform&);
    bool DrawVec3Component(
        const std::string& label , glm::vec3& vec ,
        bool& manual_input , float rest_val 
    );
    
    void ScenePanel::DrawEntity(YE::Entity* entity) {
        auto& id  = entity->GetComponent<YE::components::ID>();
        auto& transform = entity->GetComponent<YE::components::Transform>();
    ...
    void ScenePanel::DrawGui(bool& open) {
        ...
        if (ImGui::BeginDragDropTargetCustom(window_rect , ImGui::GetCurrentWindow()->ID)) {
            ...
            if (payload != nullptr) {
                size_t count = payload->DataSize / sizeof(YE::UUID);

                for (size_t i = 0; i < count; ++i) {
                    YE::UUID id = *(((YE::UUID*)payload->Data) + i);
                    YE::Entity* ent = context->GetEntity(id);
                    if (ent != nullptr) {
                        // move entity
                    } else {
    ...
    void DrawTransformComponent(YE::components::Transform& transform) {
    ...
    bool DrawVec3Component(
        ...
        
        {
            ...
            auto item_drawer = [&](
                ...
                if (modified && YE::Keyboard::KeyDown(YE::Keyboard::Key::YE_TAB))
                    manual_input = true;
